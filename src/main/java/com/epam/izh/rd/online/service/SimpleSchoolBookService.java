...
public class SimpleSchoolBookService implements BookService<SchoolBook> {
    ...
    public Author findAuthorByBookName(String name) {
        SchoolBook[] forSearch = schoolBookBookRepository.findByName(name);
        if (forSearch.length == 0) {
            return null;
        }
        return authorService.findByFullName(forSearch[0].getAuthorName(), forSearch[0].getAuthorLastName());
    }
}
